...
use bevy_ecs::{prelude::not, schedule::IntoSystemConfigs, system::{Res, ResMut}};
use bevy_log::debug;
...
impl BlockModelProcessor {
    ...
    fn create_model(
        ...
        let elements = Self::get_elements(definition, catalog, definitions)?;
        ...
        for element in elements {
            ...
            for direction in Self::CUBOID_FACES {
                ...
                if let Some(texture_handle) = Self::get_element_texture(
                    ...
                    if let Some(atlas_index) =
                        atlas.layout().get_texture_index(texture_handle.id().typed_debug_checked())
                    {
