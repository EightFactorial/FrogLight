...
impl<T: FrogRead, const N: usize> FrogRead for [T; N] {
    ...
    where
        Self: Sized,
    {
        core::array::try_from_fn(|_| T::frog_read(buf))
    }
}
...
impl<T: FrogRead, const N: usize> FrogRead for SmallVec<[T; N]> {
    ...
    where
        Self: Sized,
    {
        Ok(SmallVec::from_vec(Vec::frog_read(buf)?))
    }
}
...
fn proto_read_array() {
    ...
    assert_eq!(<[u8; 4]>::frog_read(&mut cursor).unwrap(), [0, 1, 0, 1]);
    assert_eq!(<[u8; 4]>::frog_read(&mut cursor).unwrap(), [0, 0, 1, 1]);

    let err = <[u8; 4]>::frog_read(&mut cursor).unwrap_err();
...
fn proto_read_vector() {
    ...
    let vec: Vec<u8> = Vec::frog_read(&mut cursor).unwrap();
    assert_eq!(vec, [1, 1]);

    let vec: Vec<u16> = Vec::frog_read(&mut cursor).unwrap();
    assert_eq!(vec, [8, 8]);

    let vec: Vec<u32> = Vec::frog_read(&mut cursor).unwrap();
...
fn proto_read_smallvec() {
    ...
    let vec: SmallVec<[u8; 4]> = SmallVec::frog_read(&mut cursor).unwrap();
    assert_eq!(vec, [1, 1].into());

    let vec: SmallVec<[u16; 4]> = SmallVec::frog_read(&mut cursor).unwrap();
    assert_eq!(vec, [8, 8].into());

    let vec: SmallVec<[u32; 4]> = SmallVec::frog_read(&mut cursor).unwrap();
