...
impl<T: FrogWrite> FrogWrite for Vec<T> {
    #[inline]
    fn frog_write(&self, buf: &mut (impl std::io::Write + ?Sized)) -> Result<(), WriteError> {
        u32::try_from(self.len()).expect("Vector length too long").frog_var_write(buf)?;
        for value in self {
            value.frog_write(buf)?;
        }
        Ok(())
...
fn proto_write_array() {
    ...
    [0, 1, 0, 1u8].frog_write(&mut buf).unwrap();
    [0, 0, 1, 1u8].frog_write(&mut buf).unwrap();
...
fn proto_write_vector() {
    ...
    vec![8u8, 8, 2, 1, 1].frog_write(&mut buf).unwrap();
...
fn proto_write_smallvec() {
    ...
    SmallVec::<[u8; 4]>::from_vec(vec![8u8, 8, 2, 1, 1]).frog_write(&mut buf).unwrap();
