...
macro_rules! impl_tuple_var_write {
    ($($name:ident),*) => {
        impl<$($name: crate::protocol::FrogVarWrite),*> crate::protocol::FrogVarWrite for ($($name,)*) {
            fn fg_var_write(&self, buf: &mut (impl std::io::Write + ?Sized)) -> Result<(), crate::protocol::WriteError> {
                let ($($name,)*) = self;
