...
impl LabelInsn {
    pub(crate) fn new(id: u32) -> Self { LabelInsn { id } }
}

#[derive(Constructor, Clone, Debug, Display, PartialEq, Eq)]
...
#[derive(Constructor, Clone, Debug, Display, PartialEq)]
pub struct LdcInsn {
...
#[derive(Constructor, Copy, Clone, Debug, Display, PartialEq, Eq)]
...
pub enum BootstrapArgument {
    ...
    Long(i64),
    Double(f64),
    Class(String), /* TODO: Continue. Do we have to do this for every constant type? Spec seems
                    * to suggest so */
}
...
impl LookupSwitchInsn {
    ...
    pub fn get(&self, case: i32) -> Option<LabelInsn> { self.cases.get(&case).cloned() }
}
...
impl TableSwitchInsn {
    ...
    pub fn get(&self, case: i32) -> Option<LabelInsn> {
        self.cases.get((case - self.low) as usize).copied()
    }
}
