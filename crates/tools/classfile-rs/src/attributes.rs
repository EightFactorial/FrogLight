use crate::{
    ast::LabelInsn,
    code::CodeAttribute,
    constantpool::{ConstantPool, ConstantPoolWriter, ConstantType},
    error::{ParserError, Result},
    utils::{MapUtils, ReadUtils},
    version::{ClassVersion, MajorVersion},
};
...
pub mod Attributes {
    use crate::{
        ast::LabelInsn,
        attributes::{Attribute, AttributeSource},
    ...
    pub fn parse<R: Read>(
        ...
        pc_label_map: &mut Option<HashMap<u32, LabelInsn>>,
    ) -> crate::Result<Vec<Attribute>> {
    ...
    pub fn write<W: Write>(
        ...
        label_pc_map: Option<&HashMap<LabelInsn, u32>>,
    ) -> crate::Result<()> {
    ...
    pub fn parse(constant_pool: &ConstantPool, buf: Vec<u8>) -> Result<Self> {
        ...
        let value = match constant_pool.get(index)? {
            ...
            ConstantType::String(x) => {
                ConstantValue::String(constant_pool.utf8(x.utf_index)?.str.clone())
            }
            x => panic!("Invalid constant value type {:#?} at index {}", x, index),
        };
        Ok(ConstantValueAttribute { value })
    }
...
impl SignatureAttribute {
    pub fn new(signature: String) -> Self { SignatureAttribute { signature } }
...
impl ExceptionsAttribute {
    pub fn new(exceptions: Vec<String>) -> Self { ExceptionsAttribute { exceptions } }
        ...
        for _ in 0..num_exceptions {
            exceptions.push(
                constant_pool
                    .utf8(
                        constant_pool
                            .class(slice.read_u16::<BigEndian>()?)?
                            .name_index,
                    )?
                    .str
                    .clone(),
...
impl UnknownAttribute {
    ...
    pub fn write<T: Write>(
        &self,
        wtr: &mut T,
        _constant_pool: &mut ConstantPoolWriter,
    ) -> Result<()> {
...
impl SourceFileAttribute {
    pub fn parse(constant_pool: &ConstantPool, buf: Vec<u8>) -> Result<Self> {
        ...
        Ok(SourceFileAttribute { source_file })
    }
...
impl LocalVariableTableAttribute {
    pub fn parse(
        ...
        Ok(LocalVariableTableAttribute { variables })
    }
    ...
    pub fn write<T: Write>(
        ...
        wtr: &mut T,
        constant_pool: &mut ConstantPoolWriter,
        label_pc_map: &HashMap<LabelInsn, u32>,
    ) -> Result<()> {
...
impl LocalVariable {
    ...
    pub fn write<T: Write>(
        ...
        let start_pc = *label_pc_map
            .get(&self.start)
            .ok_or_else(ParserError::unmapped_label)?;
        wtr.write_u16::<BigEndian>(start_pc as u16)?;
        let end_pc = *label_pc_map
            .get(&self.end)
            .ok_or_else(ParserError::unmapped_label)?;
...
impl Attribute {
    pub fn parse<R: Read>(
        ...
        version: &ClassVersion,
        constant_pool: &ConstantPool,
        pc_label_map: Option<&mut HashMap<u32, LabelInsn>>,
    ) -> Result<Attribute> {
        let name = constant_pool
            .utf8(rdr.read_u16::<BigEndian>()?)?
            .str
            .clone();
        let attribute_length = rdr.read_u32::<BigEndian>()? as usize;
        let buf: Vec<u8> = rdr.read_nbytes(attribute_length)?;
        let str = name.as_str();
        ...
        let attr = match source {
            ...
            AttributeSource::Code => {
                ...
                if str == "LocalVariableTable" {
                    Attribute::LocalVariableTable(LocalVariableTableAttribute::parse(
                        constant_pool,
                        buf,
                        pc_label_map,
                    )?)
