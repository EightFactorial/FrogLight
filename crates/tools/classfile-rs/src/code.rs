use crate::{
    ...
    constantpool::{CPIndex, ConstantPool, ConstantPoolWriter, ConstantType},
    error::{ParserError, Result},
    insnlist::InsnList,
    types::{parse_method_desc, Type},
    utils::{MapUtils, ReadUtils},
...
impl CodeAttribute {
    ...
    pub fn parse(
        ...
        code.set_position(0);
        let code = InsnParser::parse_insns(constant_pool, &mut code, code_length, &pc_label_map)?;
    ...
    pub fn write<T: Write>(
        &self,
        wtr: &mut T,
        constant_pool: &mut ConstantPoolWriter,
    ) -> Result<()> {
...
impl ExceptionHandler {
    ...
    pub fn parse<T: Read>(constant_pool: &ConstantPool, buf: &mut T) -> Result<Self> {
        ...
        let catch_type = if catch_index > 0 {
            Some(
                constant_pool
                    .utf8(constant_pool.class(catch_index)?.name_index)?
                    .str
                    .clone(),
            )
        } else {
...
impl InsnParser {
    ...
    /// Iterate all instructions and collect any pcs that are referenced - i.e. need to have
    /// relevant Labels
    fn find_insn_refs<T: Read + Seek>(
        rdr: &mut T,
        length: u32,
        pc_label_map: &mut HashMap<u32, LabelInsn>,
    ) -> Result<()> {
        let mut pc: u32 = 0;
        while pc < length {
            let this_pc = pc;
            ...
            match opcode {
                InsnParser::GOTO => {
                    ...
                    pc_label_map
                        .insert_if_not_present(to, LabelInsn::new(pc_label_map.len() as u32));
                    pc += 2;
                }
                InsnParser::GOTO_W => {
                ...
                InsnParser::LOOKUPSWITCH => {
                    ...
                    pc_label_map
                        .insert_if_not_present(default, LabelInsn::new(pc_label_map.len() as u32));
                    ...
                    for i in 0..npairs {
                        ...
                        pc_label_map
                            .insert_if_not_present(jump, LabelInsn::new(pc_label_map.len() as u32));
                    }
                ...
                InsnParser::TABLESWITCH => {
                    ...
                    for i in 0..num_cases {
                        ...
                        pc_label_map
                            .insert_if_not_present(case, LabelInsn::new(pc_label_map.len() as u32));
                    }
                ...
                InsnParser::AALOAD
                | InsnParser::AASTORE
                | InsnParser::ACONST_NULL
                | InsnParser::ALOAD_0
                | InsnParser::ALOAD_1
                | InsnParser::ALOAD_2
                | InsnParser::ALOAD_3
                | InsnParser::ARETURN
                | InsnParser::ARRAYLENGTH
                | InsnParser::ASTORE_0
                | InsnParser::ASTORE_1
                | InsnParser::ASTORE_2
                | InsnParser::ASTORE_3
                ...
                | InsnParser::LUSHR
                | InsnParser::LXOR
                | InsnParser::MONITORENTER
                | InsnParser::MONITOREXIT
                | InsnParser::NOP
                | InsnParser::POP
                | InsnParser::POP2
                | InsnParser::RETURN
                ...
                InsnParser::WIDE => match rdr.read_u8()? {
                    ...
                    _ => {
                        return Err(ParserError::invalid_insn(
                            this_pc,
                            format!("Invalid wide opcode {:x}", opcode),
                        ))
                    }
                },
    ...
    fn parse_insns<T: Read>(
        constant_pool: &ConstantPool,
        mut rdr: T,
        length: u32,
        pc_label_map: &HashMap<u32, LabelInsn>,
    ) -> Result<InsnList> {
        ...
        while pc < length {
            ...
            let insn = match opcode {
                ...
                // BALOAD is both byte and boolean (they are same size on hotspot) we will assume
                // byte
                ...
                InsnParser::D2F => Insn::Convert(ConvertInsn::new(
                    PrimitiveType::Double,
                    PrimitiveType::Float,
                )),
                InsnParser::D2I => {
                    Insn::Convert(ConvertInsn::new(PrimitiveType::Double, PrimitiveType::Int))
                }
                InsnParser::D2L => {
                    Insn::Convert(ConvertInsn::new(PrimitiveType::Double, PrimitiveType::Long))
                }
                ...
                InsnParser::F2D => Insn::Convert(ConvertInsn::new(
                    PrimitiveType::Float,
                    PrimitiveType::Double,
                )),
                InsnParser::F2I => {
                    Insn::Convert(ConvertInsn::new(PrimitiveType::Float, PrimitiveType::Int))
                }
                InsnParser::F2L => {
                    Insn::Convert(ConvertInsn::new(PrimitiveType::Float, PrimitiveType::Long))
                }
                ...
                InsnParser::GETFIELD => {
                    ...
                    let class = constant_pool
                        .utf8(constant_pool.class(field_ref.class_index)?.name_index)?
                        .str
                        .clone();
                ...
                InsnParser::GOTO => {
                    let to = (rdr.read_i16::<BigEndian>()? as i32 + this_pc as i32) as u32;
                    pc += 2;
                    Insn::Jump(JumpInsn::new(
                        *pc_label_map
                            .get(&to)
                            .ok_or_else(ParserError::unmapped_label)?,
                    ))
                }
                InsnParser::GOTO_W => {
                    let to = (rdr.read_i32::<BigEndian>()? + this_pc as i32) as u32;
                ...
                InsnParser::I2B => {
                    Insn::Convert(ConvertInsn::new(PrimitiveType::Int, PrimitiveType::Byte))
                }
                InsnParser::I2C => {
                    Insn::Convert(ConvertInsn::new(PrimitiveType::Int, PrimitiveType::Char))
                }
                InsnParser::I2D => {
                    Insn::Convert(ConvertInsn::new(PrimitiveType::Int, PrimitiveType::Double))
                }
                InsnParser::I2F => {
                    Insn::Convert(ConvertInsn::new(PrimitiveType::Int, PrimitiveType::Float))
                }
                InsnParser::I2L => {
                    Insn::Convert(ConvertInsn::new(PrimitiveType::Int, PrimitiveType::Long))
                }
                InsnParser::I2S => {
                    Insn::Convert(ConvertInsn::new(PrimitiveType::Int, PrimitiveType::Short))
                }
                ...
                InsnParser::IF_ACMPEQ => {
                    let to = (rdr.read_i16::<BigEndian>()? as i32 + this_pc as i32) as u32;
                    pc += 2;
                    Insn::ConditionalJump(ConditionalJumpInsn::new(
                        JumpCondition::ReferencesEqual,
                        *pc_label_map
                            .get(&to)
                            .ok_or_else(ParserError::unmapped_label)?,
                    ))
                ...
                InsnParser::IF_ACMPNE => {
                    ...
                    pc += 2;
                    Insn::ConditionalJump(ConditionalJumpInsn::new(
                        JumpCondition::ReferencesNotEqual,
                        *pc_label_map
                            .get(&to)
                            .ok_or_else(ParserError::unmapped_label)?,
                    ))
                ...
                InsnParser::IF_ICMPGE => {
                    ...
                    pc += 2;
                    Insn::ConditionalJump(ConditionalJumpInsn::new(
                        JumpCondition::IntsGreaterThanOrEq,
                        *pc_label_map
                            .get(&to)
                            .ok_or_else(ParserError::unmapped_label)?,
                    ))
                ...
                InsnParser::IF_ICMPLE => {
                    ...
                    pc += 2;
                    Insn::ConditionalJump(ConditionalJumpInsn::new(
                        JumpCondition::IntsLessThanOrEq,
                        *pc_label_map
                            .get(&to)
                            .ok_or_else(ParserError::unmapped_label)?,
                    ))
                ...
                InsnParser::IFGE => {
                    ...
                    pc += 2;
                    Insn::ConditionalJump(ConditionalJumpInsn::new(
                        JumpCondition::IntGreaterThanOrEqZero,
                        *pc_label_map
                            .get(&to)
                            .ok_or_else(ParserError::unmapped_label)?,
                    ))
                ...
                InsnParser::IFGT => {
                    ...
                    pc += 2;
                    Insn::ConditionalJump(ConditionalJumpInsn::new(
                        JumpCondition::IntGreaterThanZero,
                        *pc_label_map
                            .get(&to)
                            .ok_or_else(ParserError::unmapped_label)?,
                    ))
                ...
                InsnParser::IFLE => {
                    ...
                    pc += 2;
                    Insn::ConditionalJump(ConditionalJumpInsn::new(
                        JumpCondition::IntLessThanOrEqZero,
                        *pc_label_map
                            .get(&to)
                            .ok_or_else(ParserError::unmapped_label)?,
                    ))
                ...
                InsnParser::INVOKEDYNAMIC => {
                    ...
                    let descriptor = constant_pool
                        ...
                        .str
                        .clone();
                    Insn::InvokeDynamic(InvokeDynamicInsn::new(
                        name,
                        descriptor,
                        BootstrapMethodType::InvokeStatic,
                ...
                InsnParser::INVOKEINTERFACE => {
                    ...
                    let class = constant_pool
                        .utf8(constant_pool.class(method.class_index)?.name_index)?
                        .str
                        .clone();
                    let name = constant_pool.utf8(name_and_type.name_index)?.str.clone();
                    let descriptor = constant_pool
                ...
                InsnParser::IUSHR => {
                    Insn::LogicalShiftRight(LogicalShiftRightInsn::new(IntegerType::Int))
                }
                InsnParser::IXOR => Insn::Xor(XorInsn::new(IntegerType::Int)),
                // InsnParser::JSR =>
                // InsnParser::JSR_W =>
                InsnParser::L2D => {
                    Insn::Convert(ConvertInsn::new(PrimitiveType::Long, PrimitiveType::Double))
                }
                InsnParser::L2F => {
                    Insn::Convert(ConvertInsn::new(PrimitiveType::Long, PrimitiveType::Float))
                }
                InsnParser::L2I => {
                    Insn::Convert(ConvertInsn::new(PrimitiveType::Long, PrimitiveType::Int))
                }
                ...
                InsnParser::LOOKUPSWITCH => {
                    ...
                    let mut insn = LookupSwitchInsn::new(
                        *pc_label_map
                            .get(&default)
                            .ok_or_else(ParserError::unmapped_label)?,
                    );
                ...
                InsnParser::LUSHR => {
                    Insn::LogicalShiftRight(LogicalShiftRightInsn::new(IntegerType::Long))
                }
                ...
                InsnParser::WIDE => {
                    ...
                    match opcode {
                        ...
                        InsnParser::RET => {
                            unimplemented!("Wide Ret instructions are not implemented")
                        }
                        _ => {
                            return Err(ParserError::invalid_insn(
                                this_pc,
    ...
    fn remap_label_nodes(
        x: &mut LabelInsn,
        list: &mut InsnList,
        pc_index_map: &HashMap<u32, u32>,
        insert: &mut HashMap<usize, Vec<Insn>>,
    ) -> Result<()> {
    ...
    fn parse_ldc(index: CPIndex, constant_pool: &ConstantPool) -> Result<Insn> {
        ...
        let ldc_type = match constant {
            ConstantType::String(x) => {
                LdcType::String(constant_pool.utf8(x.utf_index)?.str.clone())
            }
            ...
            ConstantType::MethodType(x) => {
                LdcType::MethodType(constant_pool.utf8(x.descriptor_index)?.str.clone())
            }
            ConstantType::MethodHandle(x) => {
                return Err(ParserError::unimplemented("MethodHandle LDC"))
            }
            ConstantType::Dynamic(x) => return Err(ParserError::unimplemented("Dynamic LDC")),
            x => {
                return Err(ParserError::incomp_cp(
                    "LDC Constant Type",
                    constant,
                    index as usize,
                ))
    ...
    fn write_insns(
        code: &CodeAttribute,
        constant_pool: &mut ConstantPoolWriter,
    ) -> Result<(Vec<u8>, HashMap<LabelInsn, u32>)> {
        ...
        for insn in code.insns.iter() {
            match insn {
                ...
                Insn::ArrayLoad(x) => {
                    wtr.write_u8(match &x.kind {
                        ...
                        Type::Void => {
                            return Err(ParserError::invalid_insn(
                                pc,
                                "Cannot use type Void in array load",
                            ))
                        }
                    })?;
                    pc = pc
                ...
                Insn::ArrayStore(x) => {
                    wtr.write_u8(match &x.kind {
                        ...
                        Type::Void => {
                            return Err(ParserError::invalid_insn(
                                pc,
                                "Cannot use type Void in array store",
                            ))
                        }
                    })?;
                    pc = pc
                ...
                Insn::Ldc(x) => {
                    pc = pc
                        .checked_add(match &x.constant {
                            LdcType::Null => {
                                wtr.write_u8(InsnParser::ACONST_NULL)?;
                                1
                            }
                            LdcType::String(x) => InsnParser::write_ldc(
                                &mut wtr,
                                constant_pool.string_utf(x.clone()),
                                false,
                            )?,
                            LdcType::Int(x) => {
                                InsnParser::write_ldc(&mut wtr, constant_pool.integer(*x), false)?
                            }
                            LdcType::Float(x) => {
                                InsnParser::write_ldc(&mut wtr, constant_pool.float(*x), false)?
                            }
                            LdcType::Long(x) => {
                                InsnParser::write_ldc(&mut wtr, constant_pool.long(*x), false)?
                            }
                            LdcType::Double(x) => {
                                InsnParser::write_ldc(&mut wtr, constant_pool.double(*x), false)?
                            }
                            LdcType::Class(x) => InsnParser::write_ldc(
                                &mut wtr,
                                constant_pool.class_utf8(x.clone()),
                                false,
                            )?,
                            LdcType::MethodType(x) => InsnParser::write_ldc(
                                &mut wtr,
                                constant_pool.methodtype_utf8(x.clone()),
                                false,
                            )?,
                            LdcType::MethodHandle() => {
                                return Err(ParserError::invalid_insn(pc, "MethodHandle LDC"))
                            }
                            LdcType::Dynamic() => {
                                return Err(ParserError::invalid_insn(pc, "Dynamic LDC"))
                            }
                        })
                        .ok_or_else(ParserError::too_many_instructions)?;
                }
                ...
                Insn::NewArray(x) => {
                    match &x.kind {
                        ...
                        Type::Void => {
                            return Err(ParserError::invalid_insn(
                                pc,
                                "Cannot use type Void in newarray",
                            ))
                        }
                    }
                }
                ...
                Insn::Convert(x) => match &x.from {
                    ...
                    PrimitiveType::Long => {
                        wtr.write_u8(match &x.to {
                            ...
                            | PrimitiveType::Boolean
                            | PrimitiveType::Byte
                            | PrimitiveType::Int => InsnParser::L2I,
                    ...
                    PrimitiveType::Float => {
                        wtr.write_u8(match &x.to {
                            ...
                            | PrimitiveType::Boolean
                            | PrimitiveType::Byte
                            | PrimitiveType::Int => InsnParser::F2I,
                    ...
                    PrimitiveType::Double => {
                        wtr.write_u8(match &x.to {
                            ...
                            | PrimitiveType::Boolean
                            | PrimitiveType::Byte
                            | PrimitiveType::Int => InsnParser::D2I,
                ...
                Insn::Divide(x) => {
                    wtr.write_u8(match &x.kind {
                        ...
                        | PrimitiveType::Char
                        | PrimitiveType::Short
                        | PrimitiveType::Int => InsnParser::IDIV,
                ...
                Insn::Multiply(x) => {
                    wtr.write_u8(match &x.kind {
                        ...
                        | PrimitiveType::Char
                        | PrimitiveType::Short
                        | PrimitiveType::Int => InsnParser::IMUL,
                ...
                Insn::Negate(x) => {
                    wtr.write_u8(match &x.kind {
                        ...
                        | PrimitiveType::Char
                        | PrimitiveType::Short
                        | PrimitiveType::Int => InsnParser::INEG,
                ...
                Insn::Remainder(x) => {
                    wtr.write_u8(match &x.kind {
                        ...
                        | PrimitiveType::Char
                        | PrimitiveType::Short
                        | PrimitiveType::Int => InsnParser::IREM,
                ...
                Insn::Subtract(x) => {
                    wtr.write_u8(match &x.kind {
                        ...
                        | PrimitiveType::Char
                        | PrimitiveType::Short
                        | PrimitiveType::Int => InsnParser::ISUB,
                    ...
                    wtr.write_u8(match x.num {
                        1 => match x.down {
                            ...
                            _ => {
                                return Err(ParserError::invalid_insn(
                                    pc,
                                    "DupInsn::down must not be larger than 2",
                                ))
                            }
                        },
                        2 => match x.down {
                        ...
                        _ => {
                            return Err(ParserError::invalid_insn(
                                pc,
                                "DupInsn::num must be in the range 1-2",
                            ))
                        }
                    })?;
                    pc = pc
                ...
                Insn::InvokeDynamic(x) => {
                    return Err(ParserError::unimplemented(
                        "Invokedynamic writing unimplemented",
                    ));
                }
                Insn::Invoke(x) => {
                    ...
                    if opcode == InsnParser::INVOKEINTERFACE {
                        ...
                        // the difference between the size of the operand stack before and after the
