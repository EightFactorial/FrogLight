...
#[allow(dead_code)]
impl ConstantPool {
    pub fn new() -> Self { ConstantPool::default() }
    ...
    pub fn class(&self, index: CPIndex) -> Result<&ClassInfo> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("Class", x, index as usize)),
        }
    }
    ...
    pub fn fieldref(&self, index: CPIndex) -> Result<&FieldRefInfo> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("FieldRef", x, index as usize)),
        }
    }
    ...
    pub fn any_method(&self, index: CPIndex) -> Result<(&MethodRefInfo, bool)> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("AnyMethodRef", x, index as usize)),
        }
    }
    ...
    pub fn methodref(&self, index: CPIndex) -> Result<&MethodRefInfo> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("MethodRef", x, index as usize)),
        }
    }
    ...
    pub fn string(&self, index: CPIndex) -> Result<&StringInfo> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("String", x, index as usize)),
        }
    }
    ...
    pub fn integer(&self, index: CPIndex) -> Result<&IntegerInfo> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("Integer", x, index as usize)),
        }
    }
    ...
    pub fn float(&self, index: CPIndex) -> Result<&FloatInfo> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("Float", x, index as usize)),
        }
    }
    ...
    pub fn long(&self, index: CPIndex) -> Result<&LongInfo> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("Long", x, index as usize)),
        }
    }
    ...
    pub fn double(&self, index: CPIndex) -> Result<&DoubleInfo> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("Double", x, index as usize)),
        }
    }
    ...
    pub fn nameandtype(&self, index: CPIndex) -> Result<&NameAndTypeInfo> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("NameAndType", x, index as usize)),
        }
    }
    ...
    pub fn utf8(&self, index: CPIndex) -> Result<&Utf8Info> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("Utf8", x, index as usize)),
        }
    }
    ...
    pub fn methodhandle(&self, index: CPIndex) -> Result<&MethodHandleInfo> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("MethodHandle", x, index as usize)),
        }
    }
    ...
    pub fn methodtype(&self, index: CPIndex) -> Result<&MethodTypeInfo> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("MethodType", x, index as usize)),
        }
    }
    ...
    pub fn dynamicinfo(&self, index: CPIndex) -> Result<&DynamicInfo> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("Dynamic", x, index as usize)),
        }
    }
    ...
    pub fn invokedynamicinfo(&self, index: CPIndex) -> Result<&InvokeDynamicInfo> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("InvokeDynamic", x, index as usize)),
        }
    }
    ...
    pub fn module(&self, index: CPIndex) -> Result<&ModuleInfo> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("Module", x, index as usize)),
        }
    }
    ...
    pub fn package(&self, index: CPIndex) -> Result<&PackageInfo> {
        match self.get(index)? {
            ...
            x => Err(ParserError::incomp_cp("Package", x, index as usize)),
        }
    }
}
...
#[derive(Constructor, Copy, Clone, Debug, Display, PartialEq, Eq, Hash)]
...
impl Display for MethodRefInfo {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result { write!(f, "{self:?}") }
}
...
impl LongInfo {
    pub fn inner(&self) -> i64 { self.inner }
}
...
impl FloatInfo {
    ...
    pub fn inner(&self) -> f32 { f32::from_bits(self.inner) }
}
...
impl DoubleInfo {
    ...
    pub fn inner(&self) -> f64 { f64::from_bits(self.inner) }
}
...
impl Display for NameAndTypeInfo {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result { write!(f, "{self:?}") }
}

#[derive(Constructor, Clone, Debug, Display, PartialEq, Eq, Hash)]
pub struct Utf8Info {
    pub str: String,
}
...
impl Display for MethodHandleInfo {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result { write!(f, "{self:?}") }
}
...
impl Display for InvokeDynamicInfo {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result { write!(f, "{self:?}") }
}
...
impl ConstantType {
    ...
    pub fn parse<R: Read>(rdr: &mut R) -> Result<Self> {
        let tag = rdr.read_u8()?;
        Ok(match tag {
            ConstantType::CONSTANT_Class => ConstantType::Class(ClassInfo {
                name_index: rdr.read_u16::<BigEndian>()?,
            }),
            ConstantType::CONSTANT_Fieldref => ConstantType::Fieldref(FieldRefInfo {
                class_index: rdr.read_u16::<BigEndian>()?,
                name_and_type_index: rdr.read_u16::<BigEndian>()?,
            }),
            ConstantType::CONSTANT_Methodref => ConstantType::Methodref(MethodRefInfo {
                class_index: rdr.read_u16::<BigEndian>()?,
                name_and_type_index: rdr.read_u16::<BigEndian>()?,
            }),
            ConstantType::CONSTANT_InterfaceMethodref => {
                ConstantType::InterfaceMethodref(MethodRefInfo {
                    class_index: rdr.read_u16::<BigEndian>()?,
                    name_and_type_index: rdr.read_u16::<BigEndian>()?,
                })
            }
            ConstantType::CONSTANT_String => ConstantType::String(StringInfo {
                utf_index: rdr.read_u16::<BigEndian>()?,
            }),
            ConstantType::CONSTANT_Integer => {
                ConstantType::Integer(IntegerInfo::new(rdr.read_i32::<BigEndian>()?))
            }
            ConstantType::CONSTANT_Float => {
                ConstantType::Float(FloatInfo::new(rdr.read_f32::<BigEndian>()?))
            }
            ConstantType::CONSTANT_Long => {
                ConstantType::Long(LongInfo::new(rdr.read_i64::<BigEndian>()?))
            }
            ConstantType::CONSTANT_Double => {
                ConstantType::Double(DoubleInfo::new(rdr.read_f64::<BigEndian>()?))
            }
            ConstantType::CONSTANT_NameAndType => ConstantType::NameAndType(NameAndTypeInfo {
                name_index: rdr.read_u16::<BigEndian>()?,
            ...
            ConstantType::CONSTANT_Utf8 => {
                ...
                let utf = match mutf8::mutf8_to_utf8(bytes.as_slice()) {
                    Ok(Cow::Borrowed(_data)) => bytes.into(),
                    Ok(Cow::Owned(data)) => data.into_boxed_slice(),
                    Err(err) => panic!("Failed to parse mutf8: {:?}", err),
                };
            ...
            ConstantType::CONSTANT_MethodType => ConstantType::MethodType(MethodTypeInfo {
                descriptor_index: rdr.read_u16::<BigEndian>()?,
            }),
            ConstantType::CONSTANT_Dynamic => ConstantType::Dynamic(DynamicInfo {
                bootstrap_method_attr_index: rdr.read_u16::<BigEndian>()?,
                name_and_type_index: rdr.read_u16::<BigEndian>()?,
            }),
            ConstantType::CONSTANT_InvokeDynamic => {
                ConstantType::InvokeDynamic(InvokeDynamicInfo {
                    bootstrap_method_attr_index: rdr.read_u16::<BigEndian>()?,
                    name_and_type_index: rdr.read_u16::<BigEndian>()?,
                })
            }
            ConstantType::CONSTANT_Module => ConstantType::Module(ModuleInfo {
                name_index: rdr.read_u16::<BigEndian>()?,
            }),
            ConstantType::CONSTANT_Package => ConstantType::Package(PackageInfo {
                name_index: rdr.read_u16::<BigEndian>()?,
            }),
        ...
        match self {
            ...
            ConstantType::Utf8(x) => {
                ...
                let mutf = match mutf8::utf8_to_mutf8(bytes) {
                    Ok(Cow::Borrowed(_data)) => bytes.into(),
                    Ok(Cow::Owned(data)) => data.into_boxed_slice(),
                    Err(err) => panic!("Failed to parse utf8: {:?}", err),
                };
...
impl ConstantPoolWriter {
    pub fn new() -> Self { ConstantPoolWriter::default() }
    ...
    pub fn len(&self) -> u16 { self.index }

    pub fn is_empty(&self) -> bool { self.index == 0 }
    ...
    pub fn fieldref(&mut self, class_index: CPIndex, name_and_type_index: CPIndex) -> CPIndex {
        self.put(ConstantType::Fieldref(FieldRefInfo::new(
            class_index,
            name_and_type_index,
        )))
    }
    ...
    pub fn methodref(&mut self, class_index: CPIndex, name_and_type_index: CPIndex) -> CPIndex {
        self.put(ConstantType::Methodref(MethodRefInfo::new(
            class_index,
            name_and_type_index,
        )))
    }

    pub fn interfacemethodref(
        &mut self,
        class_index: CPIndex,
        name_and_type_index: CPIndex,
    ) -> CPIndex {
        self.put(ConstantType::InterfaceMethodref(MethodRefInfo::new(
            class_index,
            name_and_type_index,
        )))
    }
    ...
    pub fn nameandtype(&mut self, name_index: CPIndex, descriptor_index: CPIndex) -> CPIndex {
        self.put(ConstantType::NameAndType(NameAndTypeInfo::new(
            name_index,
            descriptor_index,
        )))
    }
    ...
    pub fn methodhandle(&mut self, kind: MethodHandleKind, reference: CPIndex) -> CPIndex {
        self.put(ConstantType::MethodHandle(MethodHandleInfo::new(
            kind, reference,
        )))
    }

    pub fn methodtype(&mut self, descriptor_index: CPIndex) -> CPIndex {
        self.put(ConstantType::MethodType(MethodTypeInfo::new(
            descriptor_index,
        )))
    }
    ...
    pub fn dynamicinfo(
        ...
        self.put(ConstantType::Dynamic(DynamicInfo::new(
            bootstrap_method_attr_index,
            name_and_type_index,
        )))
    }

    pub fn invokedynamicinfo(
        &mut self,
        bootstrap_method_attr_index: CPIndex,
        name_and_type_index: CPIndex,
    ) -> CPIndex {
        self.put(ConstantType::InvokeDynamic(InvokeDynamicInfo::new(
            bootstrap_method_attr_index,
            name_and_type_index,
        )))
    }
