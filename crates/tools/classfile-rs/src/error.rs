...
use std::{fmt::Debug, io, result, str::Utf8Error, string::FromUtf8Error};
use thiserror::Error;
...
pub enum ParserError {
    ...
    InvalidInstruction { pc: u32, msg: String },
...
impl ParserError {
    ...
    pub fn io(inner: io::Error) -> Self { ParserError::IO(inner).check_panic() }
    ...
    pub fn none(name: &'static str) -> Self { ParserError::None(name).check_panic() }
    ...
    pub fn invalid_utf8(err: Utf8Error) -> Self { ParserError::InvalidUtf8(err).check_panic() }

    pub fn too_many_instructions() -> Self { ParserError::TooManyInstructions().check_panic() }
    ...
    pub fn unmapped_label() -> Self { ParserError::other("No mapping found for label") }
}

impl From<io::Error> for ParserError {
    fn from(inner: io::Error) -> Self { ParserError::io(inner) }
}

impl From<std::string::FromUtf8Error> for ParserError {
    fn from(err: FromUtf8Error) -> Self { ParserError::invalid_utf8(err.utf8_error()) }
}
