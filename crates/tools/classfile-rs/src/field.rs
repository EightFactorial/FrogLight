use crate::{
    ...
    attributes::{Attribute, AttributeSource, Attributes, SignatureAttribute},
...
impl Field {
    ...
    pub fn set_signature(&mut self, sig: Option<String>) {
        // According to the JVM spec there must be at most one signature attribute in the attributes
        // table first find the index of the existing sig
        let index = self
            .attributes
            .find_first(|attr| matches!(attr, Attribute::Signature(_)));
        if let Some(sig) = sig {
