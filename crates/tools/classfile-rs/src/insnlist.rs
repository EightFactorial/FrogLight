...
#[allow(dead_code)]
impl InsnList {
    pub fn new() -> Self { InsnList::default() }
    ...
    pub fn iter(&self) -> Iter<'_, Insn> { self.insns.iter() }

    pub fn len(&self) -> usize { self.insns.len() }
...
impl Debug for InsnList {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_list().entries(&self.insns).finish()
    }
}
