use crate::{
    ...
    attributes::{Attribute, AttributeSource, Attributes, ExceptionsAttribute, SignatureAttribute},
    code::CodeAttribute,
...
impl Method {
    ...
    pub fn set_exceptions(&mut self, exc: Option<Vec<String>>) {
        let index = self
            .attributes
            .find_first(|attr| matches!(attr, Attribute::Exceptions(_)));
        if let Some(exc) = exc {
    ...
    pub fn set_code(&mut self, code: Option<CodeAttribute>) {
        let index = self
            .attributes
            .find_first(|attr| matches!(attr, Attribute::Code(_)));
