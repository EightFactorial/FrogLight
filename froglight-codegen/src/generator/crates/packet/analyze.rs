...
impl PacketData {
    ...
    pub(super) async fn analyze_packet_protocol(
        ...
        analyze_codec_init(packet, class, &codec, init.as_slice(), init.last().unwrap(), jar)?;
...
fn analyze_codec_init(
    ...
    match invoke {
        ...
        | Opcode::Invokestatic(MemberRef { class_name, .. }))
            if !matches!(
                class_name.as_ref(),
                "net/minecraft/network/codec/StreamCodec" | "net/minecraft/network/protocol/Packet"
            ) =>
        {
            tracing::warn!(
        ...
        Opcode::Invokestatic(MemberRef { name_and_type, .. }) if name_and_type.name == "codec" => {
            ...
            if let (Opcode::Invokedynamic(encode), Opcode::Invokedynamic(decode)) = (encode, decode)
            {
                analyze_codec_dynamic_encode(packet, class, encode, jar)?;
                analyze_codec_dynamic_decode(packet, class, decode, jar)?;
            } else {
                miette::bail!(
        ...
        Opcode::Invokestatic(MemberRef { name_and_type, .. }) if name_and_type.name == "unit" => {}
